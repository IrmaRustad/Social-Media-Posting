...
def cargar_video(video_id, video_nombre):
    print(f"Ejecutando script de carga de video para Video ID {video_id} - Nombre: {video_nombre}")
    subir_video_instagram()

# Conectar a la base de datos (ajusta la ruta y el nombre de la base de datos según sea necesario)
conexion = sqlite3.connect('C:/Users/irma/Downloads/ig/videos.db')
...
for video in videos_por_programar:
    ...
    schedule.every().day.at(hora_programada.strftime('%H:%M')).do(cargar_video, video_id=video_id, video_nombre=video_nombre)
